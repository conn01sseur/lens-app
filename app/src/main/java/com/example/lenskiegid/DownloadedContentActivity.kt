...
class DownloadedContentActivity : BaseEdgeToEdgeActivity() {
    ...
    private fun startRouteDownload() {
        ...
        if (!AppEnv.hasInternet(this)) {
            ...
            showToast("Нет интернета для скачивания маршрута")
            return
        }
    ...
    private fun startTileDownload() {
        ...
        if (!AppEnv.hasInternet(this)) {
            ...
            showToast("Нет интернета для скачивания офлайн-карты")
            return
        }
        ...
        tileJob = scope.launch {
            try {
                ...
                showToast("Офлайн-карта скачана")
            } catch (_: Exception) {
                updateTileState("error")
                showToast("Ошибка скачивания офлайн-карты")
            }
        }
    }
