...
class MainMenuActivity : BaseEdgeToEdgeActivity() {
    ...
    private fun loadWeatherForMenu() {
        if (!AppEnv.hasInternet(this)) {
            if (!renderAnyCachedWeatherInMenu()) {
                showToast("Нет интернета для обновления погоды.")
            }
            return
        }
    ...
    private fun fetchAndApplyWeather(location: Location) {
        ...
        weatherScope.launch {
            ...
            if (data == null) {
                if (!renderAnyCachedWeatherInMenu()) {
                    showToast("Не удалось обновить погоду в меню.")
                }
                return@launch
            }
            val json = JSONObject()
    ...
    private fun renderAnyCachedWeatherInMenu(): Boolean {
        val cached = WeatherCache.getAnyJson(this) ?: return false
        pendingWeatherJson = cached
        menuWebView.evaluateJavascript("window.updateWeatherCard($cached);", null)
        return true
    }
